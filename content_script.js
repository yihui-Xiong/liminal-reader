...
(function () {
  ...
  try {
    ...
    console.warn("[Liminal Reader] Readability threw:", e);
  }
  ...
  if (!article || (article.textContent || "").trim().length < MIN_CHARS) {
    console.warn("[Liminal Reader] Readability result too short, trying fallback extraction.");
    article = fallbackExtract() || article;
  }

  if (!article || (article.textContent || "").trim().length < MIN_CHARS) {
    console.warn("[Liminal Reader] Could not extract enough content.");
    showUnsupportedMessage(
  ...
  console.group("[Liminal Reader] Extraction result");
  ...
  console.log("[Liminal Reader] content_script done — host ready for reader.js");
