...
(function () {
  ...
  if (!host) { console.error("[Liminal Reader] reader.js: host not found."); return; }
  const shadow = host.shadowRoot;
  if (!shadow) { console.error("[Liminal Reader] reader.js: no shadow root."); return; }
  const data = host.__articleData;
  if (!data) { console.error("[Liminal Reader] reader.js: no article data."); return; }

  console.log("[Liminal Reader] reader.js starting, title:", data.title);
  ...
  console.log(`[Liminal Reader] ${paragraphs.length} paragraphs, ${contentBlocks.filter(b=>b.type==="image").length} images`);
  ...
  console.log(`[Liminal Reader] reader.js ready — ${paragraphs.length} paragraphs.`);
